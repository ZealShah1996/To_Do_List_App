 ...
 */
router.get('/:todolistitemid?', async function (req, res) {
  try {
    let requiredFields = await utilityService.findRequiredFieldsFromHeaders(req, requiredFieldsForGetALl);
    let data;
    // = await mongoService.findAll(modelName, { "is_active": true, "is_deleted": false }, { "requiredFields": requiredFields });
   
    if(req.params.todolistitemid>0){
    data= await mongoService.findAll(modelName,{"is_active":true,"is_deleted":false,"id":req.params.todolistitemid},{"requiredFields":requiredFields});
    }else{
      data=await mongoService.findAll(modelName,{"is_active":true,"is_deleted":false},{"requiredFields":requiredFields});
    }                     
